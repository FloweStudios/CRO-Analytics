// Generates the complete, copy-pasteable tracker snippet for a partner.
// The snippet is self-contained — no external script tag needed.
// Domain locking is enforced inside the snippet itself.
...
(function(){
  ...
  // ── Domain lock ─────────────────────────────────────────────────
  // This snippet will ONLY fire on the authorised domain.
  // Remove or change the domain check and events stop being recorded.
  ...
  var FLUSH_MS   = 10000;
  var STORE_KEY  = '__cro_q_${clientId}__';
  var MAX_Q      = 100;
  ...
  function xp(el){
    ...
    while(node&&node.nodeType===1&&node!==document.body){
      ...
      var sibs=node.parentNode?Array.prototype.filter.call(
        node.parentNode.childNodes,function(n){
          return n.nodeType===1&&n.tagName===node.tagName;
        }):[];
  ...
  function flush(beacon){
    ...
    if(beacon&&navigator.sendBeacon){
      ...
      if(navigator.sendBeacon(INGEST_URL,b))saveQ([]);
    }else{
