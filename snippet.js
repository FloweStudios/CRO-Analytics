...
function Dashboard({ session }) {
  ...
  const [loading, setLoading] = useState(true);
  const [view, setView] = useState('partners');          // 'partners' | 'partner-detail'
    ...
    <div className="dash-root">
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          <button
            className={`nav-item ${view === 'partners' || view === 'partner-detail' ? 'active' : ''}`}
            onClick={goBack}
          >
            <IconGrid /> Partners
          </button>
...
function PartnersPage({ partners, loading, onOpen, onCreated, showCreate, setShowCreate, onDeleted }) {
  return (
    <div className="page">
      ...
      {loading ? (
        ...
        <div className="partner-grid">
          {partners.map(p => (
            <PartnerCard key={p.id} partner={p} onClick={() => onOpen(p)} />
          ))}
        </div>
      )}
    </div>
  ...
  return (
    <button className="partner-card" onClick={onClick}>
      ...
      <div className="card-stats">
        <Stat label="Pageviews" value={stats ? fmt(stats.pageviews) : '—'} />
        <Stat label="Clicks" value={stats ? fmt(stats.clicks) : '—'} />
        <Stat label="Period" value="30d" />
      </div>
    </button>
  );
}
...
function CreatePartnerModal({ onClose, onCreated }) {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    const { data, error } = await createPartner({ name: name.trim(), domain: d });
    if (error) {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="modal-actions">
            ...
            <button type="submit" className="btn-primary" disabled={loading}>
              {loading ? <span className="spinner" /> : 'Create partner'}
            </button>
          </div>
        </form>
      </div>
...
function PartnerDetail({ partner, onBack, onDeleted }) {
  ...
  const [stats, setStats] = useState(null);
  const [activeTab, setActiveTab] = useState('snippet'); // 'snippet' | 'settings'
  ...
  return (
    <div className="page">
      ...
      <div className="detail-header">
        ...
        <div className="detail-title-row">
          ...
          <div>
            ...
            <a className="card-domain link" href={`https://${partner.domain}`} target="_blank" rel="noreferrer">
              {partner.domain} ↗
            </a>
          </div>
        </div>
      ...
      <div className="stats-strip">
        <StatBox label="Pageviews (30d)" value={stats ? fmt(stats.pageviews) : '—'} />
        <StatBox label="Clicks (30d)" value={stats ? fmt(stats.clicks) : '—'} />
        <StatBox label="Partner ID" value={partner.id} mono />
        <StatBox label="Domain lock" value={partner.domain} mono />
      </div>
      ...
      {activeTab === 'snippet' && (
        <div className="snippet-section">
          <div className="snippet-intro">
            <p>Paste this snippet into the <code>&lt;head&gt;</code> of every page on <strong>{partner.domain}</strong>. It will only fire on that domain — any other site is silently ignored.</p>
          </div>
          ...
          <div className="snippet-notes">
            <div className="note-row">
              <span className="note-pill green">Domain locked</span>
              <span>Only fires on <code>{partner.domain}</code> and its subdomains</span>
            </div>
            <div className="note-row">
              <span className="note-pill blue">Non-blocking</span>
              <span>Loads asynchronously — zero impact on page speed or Core Web Vitals</span>
            </div>
            <div className="note-row">
              <span className="note-pill amber">Buffered</span>
              <span>Events queue locally and flush every 10s — survives network blips and tab closes</span>
            </div>
            <div className="note-row">
              <span className="note-pill gray">Idempotent</span>
              <span>Retried batches are deduplicated server-side — no duplicate data</span>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="settings-section">
          <div className="settings-group">
            ...
            <div className="settings-row">
              ...
              <span className="settings-val">{partner.name}</span>
            </div>
            <div className="settings-row">
              <span className="settings-key">Domain</span>
              <span className="settings-val mono">{partner.domain}</span>
            </div>
            <div className="settings-row">
              <span className="settings-key">Partner ID</span>
              <span className="settings-val mono">{partner.id}</span>
            </div>
            <div className="settings-row">
              <span className="settings-key">Created</span>
              <span className="settings-val">{new Date(partner.created_at).toLocaleDateString()}</span>
            </div>
          </div>
...
function StatBox({ label, value, mono }) {
  return (
    <div className="stat-box">
      <span className={`stat-box-val ${mono ? 'mono' : ''}`}>{value}</span>
