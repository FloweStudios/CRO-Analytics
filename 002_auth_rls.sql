// api/ingest.js
// Vercel Serverless Function — receives batched events from tracker.js
// Vercel uses named exports instead of exports.handler
...
// Vercel signature: (req, res) — same as Express
export default async function handler(req, res) {
  ...
  if (clientErr || !client || client.secret_key !== secret) {
    return res.status(202).send('accepted'); // don't leak whether client exists
  }
  ...
  if (valid.length > 0) {
    ...
    if (error) console.error('[ingest] Supabase error:', error.message);
  }
